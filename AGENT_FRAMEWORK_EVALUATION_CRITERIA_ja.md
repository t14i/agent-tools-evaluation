...
**責務: モック・状態注入・シミュレーション・Dry Run・Evals** — 作ったエージェントが壊れていないことを本番投入前にどう保証するか。LLMアプリは確率的に動くため、従来の単体テスト以上にフレームワーク側の支援が必要。
...
| カテゴリ | ID | 項目 | LangGraph | CrewAI | Smooth | 備考（Doc / Repro / Evidence） |
|---------|-----|------|-----------|--------|--------|-------------------------------|
| TC | TC-01 | Tool定義 | | | | |
| ... | ... | ... | ... | ... | ... | ... |
